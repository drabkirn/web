...
RSpec.describe Ui::Users::MultiFactorAuthenticationController, type: :controller do
  ...
  describe "POST #verify_enable" do
    ...
    context "when could not be enabled" do
      ...
      context "If user is Omniauth user" do
        before(:each) do
          sign_in github_user
          post :verify_enable, params: { id: github_user.id, multi_factor_authentication: { otp_code_token: github_user.otp_code } }
        end
        ...
        it "shows 2fa not enabled message" do
          expect(flash[:alert]).to eq Message.two_fa_for_non_omniauth_user(github_user.provider)
        end
      end
  ...
  describe "POST #verify_disabled" do
    ...
    context "when could not be disabled" do
      ...
      context "If user is Omniauth user" do
        before(:each) do
          sign_in github_user
          post :verify_disable, params: { id: github_user.id, multi_factor_authentication: { otp_code_token: github_user.otp_code } }
        end
